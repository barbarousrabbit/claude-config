...
((cssText, artDataUrl, videoDataUrl, themeConfig) => {
  ...
  const ROOT_ATTRS = [
    ...
    "data-dream-art-ready", "data-dream-media", "data-dream-video-active",
    "data-dream-sidebar",
  ];
  const VIDEO_ID = "codex-dream-skin-video";
  const SIDEBAR_TOGGLE_ID = "codex-dream-skin-sidebar-toggle";
  const SIDEBAR_MODE_KEY = "codex-dream-skin-sidebar-mode";
  ...
  // Optional looping video wallpaper. Absent themes keep the static art path
  // untouched, so a missing or malformed data URL degrades to image-only.
  const videoUrl = (() => {
    if (typeof videoDataUrl !== "string" || !videoDataUrl.startsWith("data:")) return null;
    try {
      const comma = videoDataUrl.indexOf(",");
      const mime = /^data:([^;,]+)/.exec(videoDataUrl)?.[1] || "video/mp4";
      const binary = atob(videoDataUrl.slice(comma + 1));
      const bytes = new Uint8Array(binary.length);
      for (let index = 0; index < binary.length; index += 1) bytes[index] = binary.charCodeAt(index);
      return URL.createObjectURL(new Blob([bytes], { type: mime }));
    } catch { return null; }
  ...
  // Muted looping wallpaper video. It is decorative only: never focusable,
  // never hit-testable, and it always degrades to the static art layer when the
  // theme ships no video or autoplay is refused.
  const ensureVideoLayer = (root) => {
    if (!videoUrl) {
      setAttribute(root, "data-dream-media", "image");
    ...
    if (!host) return;
    let node = document.getElementById(VIDEO_ID);
    if (!node || !host.contains(node)) {
      ...
      // Attributes matter as much as properties: Chromium gates autoplay on the
      // parsed muted/playsinline attributes, not only the DOM properties.
    ...
    if (node.getAttribute("src") !== videoUrl) node.src = videoUrl;
    if (node.paused) {
      const started = node.play();
      if (started && typeof started.then === "function") {
        started.then(
          () => setAttribute(root, "data-dream-video-active", "true"),
          () => setAttribute(root, "data-dream-video-active", "false"),
        );
      }
    } else {
      setAttribute(root, "data-dream-video-active", "true");
  ...
  // Sidebar transparency toggle. Only meaningful over a video wallpaper, so the
  // control is created solely for video themes and removed otherwise.
  const readSidebarMode = () => {
    try {
      const stored = localStorage.getItem(SIDEBAR_MODE_KEY);
      return stored === "opaque" || stored === "translucent" ? stored : "translucent";
    } catch { return "translucent"; }
  };

  const labelForMode = (mode) => (mode === "opaque" ? "侧栏 不透明" : "侧栏 透明");

  const ensureSidebarToggle = (root) => {
    if (!videoUrl) {
      document.getElementById(SIDEBAR_TOGGLE_ID)?.remove();
      root.removeAttribute("data-dream-sidebar");
      return;
    }
    const mode = readSidebarMode();
    setAttribute(root, "data-dream-sidebar", mode);
    const host = document.body;
    if (!host) return;
    let button = document.getElementById(SIDEBAR_TOGGLE_ID);
    if (!button || !host.contains(button)) {
      ...
      button.addEventListener("click", (event) => {
        ...
        const next = readSidebarMode() === "translucent" ? "opaque" : "translucent";
        try { localStorage.setItem(SIDEBAR_MODE_KEY, next); } catch {}
        setAttribute(document.documentElement, "data-dream-sidebar", next);
        button.textContent = labelForMode(next);
        button.setAttribute("aria-pressed", next === "opaque" ? "true" : "false");
      });
      host.appendChild(button);
    }
    button.textContent = labelForMode(mode);
    button.setAttribute("aria-pressed", mode === "opaque" ? "true" : "false");
    button.setAttribute("aria-label", "切换侧栏透明度");
    button.setAttribute("title", "切换侧栏透明度");
  };
