# UI Design Protocol — Reference Detail

Loaded on demand when the `## UI Design Protocol` stub in CLAUDE.md fires (a NEW page/route, a NEW reusable component, or a restyle of a component's overall look). The stub holds the always-on rules; this file holds the execution detail.
...
- If the script fails or the user is in a hurry, at minimum manually decide: palette (3-5 colors with hex), font pair, and overall tone (e.g. "warm minimal", "dark editorial")
- NEVER skip this step — a page without a design system looks like AI slop
...
- Read the reference files in `frontend-design/reference/` for detail:
...
- [ ] Responsive, mobile-first (defer exact breakpoints to `frontend-design`)
- [ ] Microcopy clear and actionable

> Note: the breakpoint list is owned by `frontend-design/reference/responsive-design.md` — do not hardcode a competing set here (a duplicated 375/768/1024 vs 375/768/1024/1440 list previously drifted out of sync).
