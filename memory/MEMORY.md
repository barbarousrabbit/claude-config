...
- [Codex skill mirror](codex-skill-mirror.md) — Codex loads only ~/.codex/skills; `mirror-skills-to-codex.sh` + `codex-mirror-skills.txt` copy Codex-format skills (agents/openai.yaml) one-way with a `.claude-mirror` marker; disable-model-invocation skills are absent from Claude's list but still installed
- [Windows power scheme restore](windows-power-scheme-restore.md) — deleted built-in plan (HP 8c5e7fda) ⇒ `duplicatescheme` fails; recreate the key with FriendlyName `powrprof.dll,-13` as SYSTEM (scheduled task — even admin is read-only there), settings fall back to DefaultPowerSchemeValues; avoid `restoredefaultschemes` (kills custom plans); `ActiveOverlayAcPowerScheme=ded574b5` = Balanced + 最佳性能 slider
- [Skill update mechanism](skill-update-mechanism.md) — `skill-sources.tsv` (repo/path/pinned/policy) + `update-skills.sh` (weekly detached check, track = 3-way merge + one commit per skill); git archive honours autocrlf (force -c core.autocrlf=false); pin single-file skills by SKILL.md body
